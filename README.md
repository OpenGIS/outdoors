...
Opens the compare app at [localhost:11000](http://localhost:11000) — a comparison map with the Outdoors style on the right and a reference style (Liberty, OpenTopoMap, or Thunderforest Outdoors) on the left. Edit `style.json` directly for quick style tweaks — Vite HMR updates instantly. Change feature flags in [`scripts/build.mjs`](scripts/build.mjs) and the watcher automatically rebuilds `style.json` and triggers HMR.
...
│   ├── style.js         # Style loader (replaces __TILEJSON_DOMAIN__ placeholders)
│   ├── map.js           # Map setup helper
│   ├── reset.css        # CSS reset
