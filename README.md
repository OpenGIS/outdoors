...
- **[OSM Liberty (OpenFreeMap fork)](https://github.com/hyperknot/openfreemap-styles)** — the base map style, a fork of [OSM Liberty](https://github.com/maputnik/osm-liberty) served via [OpenFreeMap](https://github.com/hyperknot/openfreemap)'s free vector tile infrastructure. Provides the full OpenMapTiles schema: land, water, roads, buildings, landuse, admin boundaries, and base POIs.
  ...
  - A **resolved copy** (`liberty-processed.json`) with `__TILEJSON_DOMAIN__` placeholders baked in is written to both `.cache/` and `dev/` — the compare app fetches it as a Liberty comparison entry (see `dev/App.vue:86`).
...
│   └── liberty-processed.json  # Generated by build script (gitignored) — served by Vite for comparison
...
│   └── liberty-processed.json  # Copied from dev/ during demo:build
├── package.json
└── vite.config.js
```
...
The compare app (`dev/App.vue`) lets you switch the left-hand reference map between multiple providers, configured in [`dev/providers.json`](dev/providers.json). Providers are grouped into three categories:
...
- **No prompt appears on page load** — the default selection always prefers providers without `apiKey` (Liberty or OpenTopoMap)

### How Liberty is loaded

1. `scripts/build.mjs` downloads the Liberty style, resolves `__TILEJSON_DOMAIN__` placeholders to `tiles.openfreemap.org`, and writes the result to `dev/liberty-processed.json`.
2. During `npm run dev`, `App.vue` fetches `./liberty-processed.json` and makes it available as the "Local Vector → Liberty" option.
3. During `npm run demo:build`, the Vite build runs first, then `dev/liberty-processed.json` is copied to `demo/` so the production build also serves it.
4. `dev/liberty-processed.json` is gitignored — it's always regenerated at build time.
