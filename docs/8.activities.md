...
> [!NOTE]
> While currently diabled, the following examples remain available behind `OUTDOOR_ROUTE` / `MTB_SCALE` feature toggles in [`scripts/build.mjs`](../scripts/build.mjs).
...
This example works well, but different rating systems exist like [mtb:scale:imba](https://wiki.openstreetmap.org/wiki/Key:mtb:scale:imba), which is are not included in the tile schema.
