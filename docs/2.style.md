...
Adds the hiking route network as overlays classified by tier — international, national, regional and local walking networks, plus a fallback tier for routes outside them — so long-distance routes read as a hierarchy. Data comes from the self-hosted route vector tiles; see [Hiking Routes](8.activities.md).
...
The shared visual language for paths — dash pattern, stroke weight, line caps and joins — applied to Liberty's base path layer and its name labels, so trails read as trails everywhere. The dashed lines use butt caps rather than round: when a line layer combines round caps with a zoom-interpolated line-width, MapLibre fails to apply `line-dasharray` and renders a solid line instead of dashed. Butt caps render the dash correctly, which is why the path family — the low-zoom overlay source (z9–13), the base `road_path_pedestrian` layer (z14+) and the overlay track layers — only renders dashed red with the shared `PATH_LINE_CAP` constant set to `butt` while `PATH_LINE_JOIN` stays round (the square dash ends are barely visible at these widths). Constant-width dashed layers, such as the ferry line (width 1.5), are unaffected and keep round caps. It owns the high-zoom range while the low-zoom paths section owns the gap below it. Configuration lives in [`scripts/build.mjs`](../scripts/build.mjs).
