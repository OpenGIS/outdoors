...
- **POI `ele` attribute** — the outdoor-POI kinds `hut`, `shelter`, `viewpoint` and `pass` carry an `ele` attribute from OSM tags (`ELE_KINDS` in [`scripts/generate-poi-schema.mjs`](../scripts/generate-poi-schema.mjs)). See [The schema generator](4.pois.md#the-schema-generator).
