...
- **Overlay** — the hosted extract. [`applyOutdoorPoi()`](../scripts/build.mjs) adds the `outdoor-poi` vector source (`https://tile.ogis.app/pois/{z}/{x}/{y}.pbf`, source-layer `outdoor_pois`, z12–16) and the `outdoor-poi` symbol layer (minzoom 12). This tier carries the kinds OMT lacks entirely (trailhead, pass, ranger_station, lighthouse, skiing…) and the kinds the overlay wants from z12 rather than OMT's z14 data floor. A `symbol-sort-key` derived from each kind's `priority` decides collision order (lower = placed first), mirroring the basemap's rank-as-sort-key convention.
- **Planet layer** — OMT data rendered directly. The `outdoor-amenities` symbol layer draws three OMT `poi` classes (doctors, bank, bicycle_rental) straight from the basemap's `openmaptiles` source at z14+, with no hosted extract. Its sort key sits at 1000 — above the overlay ladder — so basemap POIs (rank 20–2000, lower = first) place first and these amenities win only against the least-important ranks.
...
[`OUTDOOR_POI`](../scripts/poi-config.mjs) drives the overlay: `sourceId`, `tileUrl`, `sourceLayer`, the zoom bounds and the `kinds` array. Each kind declares:
...
- **priority** — the symbol-sort-key weight (lower = placed first = wins collisions), mirroring the basemap's rank-as-sort-key convention. The ladder runs 10 (outdoor identity: hut, pass, trailhead, ranger_station) → 50 (information, parking); [`scripts/poi-config.mjs`](../scripts/poi-config.mjs) is the source of truth for the full mapping.
...
Each entry declares `class`, `icon`, `showTitle` (whether the label includes the name) and `priority`. The icons live in the basemap's default sprite, with an `outdoors:dot` fallback that never renders since every class has an explicit icon. The planet priority is 1000 — deliberately above the overlay ladder: the basemap's own POI layers sort by feature rank (20–2000, lower = first), so a ladder value would let doctors/bank/bicycle_rental outrank nearly every basemap POI they collide with. 1000 keeps basemap POIs first, these amenities winning only against the least-important ranks (≥1000).

## Handoff

Each overlay kind renders only below its handoff zoom (`POI_FILTER`); at/above it the basemap's own POI layer named in the table takes over (Attraction z15, Cultural z15, Ferry z15, Local park z15, Campsite z16, Outdoor z16, Public z16, Accommodation z17, Transport z17, Waste z18), so there are no duplicates and no gaps. Labels are `name` plus elevation for the `showEle` kinds (`POI_TEXT_EXPR`); icons map kind → sprite glyph via `POI_ICON_MATCH`, with `outdoorIcon()` prefixing the outdoors-owned icons and an `outdoors:dot` fallback that never renders, since every kind has an explicit icon. Collision order comes from `POI_SORT_KEY_MATCH` — a match over kind → priority (lower = placed first), with a 50 fallback that never fires because the filter only admits configured kinds.
...
> [!NOTE]
> **Deploy dependency** — the outdoors-owned icons (see `OUTDOOR_SPRITE_ICONS` in [`scripts/build.mjs`](../scripts/build.mjs)) live in the outdoors sheet and are referenced with the `outdoors:` prefix; the sheet is served from an absolute URL (`https://www.ogis.org/outdoors/sprite`) that only exists once the demo deploys — the icons render on the deployed site. See [Extending the Icon Set](#extending-the-icon-set).

## Schema — `pois/pois-schema.yml`

`npm run pois:schema` runs [`scripts/generate-poi-schema.mjs`](../scripts/generate-poi-schema.mjs), which reads `OUTDOOR_POI` and writes [`pois/pois-schema.yml`](../pois/pois-schema.yml) — a **GENERATED** Planetiler YAML schema (do not edit by hand): one feature per kind, centroid geometry, `min_zoom` 12, attributes `kind`/`name` (plus `ele` for `showEle` kinds), and `include_when` OSM-tag selectors per kind from the `INCLUDE_WHEN` map. The extract area defaults to `geofabrik:italy`, overridable with `--area=` or the `POI_AREA` environment variable.

## Checker — `npm run check:pois`

[`scripts/check-poi-coverage.mjs`](../scripts/check-poi-coverage.mjs) runs five checks against the built `style.json`, the config and the schema, exiting 1 on any failure:
...
3. **Sort priority** — every kind in `OUTDOOR_POI.kinds` carries a numeric `priority`, and the `outdoor-poi` layer's `symbol-sort-key` is a match over `kind` covering every kind at its config priority.
4. **Schema sync** — the kind set in `pois/pois-schema.yml` matches `OUTDOOR_POI.kinds`.
5. **Planet layer** — the `outdoor-amenities` layer exists with the correct source/source-layer/minzoom, its filter contains every class in `PLANET_POI.kinds`, and its `symbol-sort-key` covers every class at its config priority (its icons are already covered by check 1).
