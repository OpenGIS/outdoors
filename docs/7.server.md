...
> The Tile Server augments existing data with outdoor POIs, low-zoom paths, hiking routes and contours.
...
- **Route relations:** paths are emitted for hiking/foot/walking route membership only — cycling/mtb deliberately excluded, in parity with the hiking routes overlay (see [Hiking Routes](8.activities.md)).
...
- [Planetiler](https://github.com/onthegomap/planetiler) — the tile generator (YAML schema + Java profile modes)
